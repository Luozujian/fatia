...
#分等级
class RevenuePredictFormateDate:
    ...
    def __init__(self):
        ...
        #self.movies_15 = self.movies_15.head(100)
        pass
    ...
    def differDirectorRevenue(self):
        ...
        self.movies1 = pd.DataFrame(columns=['director','revenue'])

        for index,row in self.movies_15.iterrows():
            if pdTemp[row['director']] >= 2:
                self.movies1 = self.movies1.append([{'director':row['director'],'revenue':row['revenue']}],ignore_index=True)

        self.movies1 = self.movies1.groupby(['director'],as_index=False)['revenue'].mean().sort_values('revenue',ascending=False)
    ...
    def diiferActorRevenue(self):
        self.movies_15 = self.movies_15.reset_index()
        ...
        self.temp = pd.DataFrame(columns=['actor','revenue'])

        for i,row in self.movies_15.iterrows():
            actorList = row['actors'].split(',')
            for item in actorList:
                actor = item.strip()
                self.temp = self.temp.append([{'actor':actor,'revenue':row['revenue']}])
        pass
        self.temp = self.temp.groupby("actor",as_index=False)['revenue'].mean().sort_values('revenue',ascending=False)
    ...
    def redExcel(self):
        ...
        for i,row in self.movies_15.iterrows():
            ...
            if 'Drama' in row['genres']:
                typeOneHot[21] = 1
            self.movies_15.loc[i,'month'] = row['release_time'][5:7]
            self.movies_15.loc[i,'typeOneHot'] = ','.join(str(i) for i in typeOneHot)
            index = index + 1
            pass
        print(index)
        if os.path.exists("complate_revenue.xlsx"):
            os.remove("complate_revenue.xlsx")

        writer = pd.ExcelWriter('complate_revenue.xlsx')
