...
class RevenuePca:
    ...
    def __init__(self):
        self.movie2 = pd.read_excel("D:\\pythonproject\\bishe\\app\\pretreatment\\date\\complate_revenue_temp.xlsx",sheet_name = "Sheet1")
        #self.movie2 = self.movie2.head(70)
        pass
    ...
    def start(self):
        self.movie2['month'] = pd.Series(self.movie2['release_time']).str[5:7]
        ...
        self.y = pd.Series(self.movie2['revenue'])
        self.y = self.y.astype("float")
        self.x = self.movie2.drop(['index','Unnamed: 0','Unnamed: 0.1.1','Unnamed: 0.1','title','release_time','genres','actor','director','revenue','typeOneHot','vote_average'],axis=1)
        ...
        # temp = mat(temp)
        # self.save(temp,"revenue_coefficient.xlsx")

        x_new = pca.transform(self.x)
        y_new = self.y.values

        sio.savemat('revenue_movie.mat',{'NIR':x_new,'octane':y_new})
        #self.x = mat(self.x)
        #self.test(self.x)
